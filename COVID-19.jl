...
function updateData()
    ...
    ### COUNTRY CODES
    # https://github.com/neherlab/covid19_scenarios_data/raw/master/country_codes.csv
    country_codes = select(CSV.read("data/country_codes.csv"), :name, Symbol("alpha-3"))

    ### LIST OF CASES
    #https://github.com/neherlab/covid19_scenarios_data/raw/master/case-counts/World.tsv
    ...
    ### ICU beds
    #https://github.com/neherlab/covid19_scenarios_data/raw/master/hospital-data/ICU_capacity.tsv
    ICU_capacity = select(CSV.read("data/ICU_capacity.tsv" delim = "\t"), :country, :CriticalCare)

    ### Hospital beds
    # https://github.com/neherlab/covid19_scenarios_data/raw/master/hospital-data/hospital_capacity.csv
    hospital_capacity = select(CSV.read("data/hospital_capacity.csv",
                                        types = Dict(:COUNTRY => String), limit = 1267),
                                        :COUNTRY, :YEAR, :VALUE)
    hospital_capacity = @where(hospital_capacity, Not(ismissing.(:COUNTRY)))

    ### Age pyramids
    # File ocnverted from JSON - No need to update
    age_distribution = CSV.read("data/country_age_distribution.csv")

    return country_codes, cases, ICU_capacity, hospital_capacity, age_distribution
end
